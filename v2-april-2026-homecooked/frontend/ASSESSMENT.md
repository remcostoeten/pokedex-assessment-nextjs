## Pokedex Assessment

A small proof-of-concept app that covers the requested Pokemon overview, capture flow, and personal Pokedex behavior.

### Tech

- Next.js
- TypeScript
- Tailwind CSS v4
- Vitest

No additional runtime packages were installed.

### Architecture

This is a toned-down version of how I usually structure applications:

- Anything reused across features goes into `shared`.
- Feature logic is colocated in `features`. I prefer this over a flat structure because UI, API access, and related actions stay grouped by concern.
- API access is wrapped in a small abstraction layer. In this project that is `create-api-connection`, plus minimal `create-mutation.ts`, `create-query.ts`, and action wrappers. Here they mainly provide structure and consistency, but in a larger application this layer would usually also handle retries, aborting, timeouts, and similar concerns.
- The UI updates optimistically so interactions feel immediate.

### Notes

- I kept the solution intentionally lightweight and package-minimal.
- The tests focus on the most important shared logic and action flow.
- With more time I would add a browser-level integration test and clearer inline error feedback for failed mutations.

Kind regards,

Remco Stoeten
